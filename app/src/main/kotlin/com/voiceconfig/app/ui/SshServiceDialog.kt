...
@Composable
fun SshServiceDialog(
    onDismiss: () -> Unit,
    defaultHost: String = "",
    initialCredential: StoredSshCredential? = null,
    result: SshExecResult? = null,
    onClearResult: () -> Unit = {},
    onList: (SshConfig) -> Unit = {},
    onStart: (SshConfig, String) -> Unit = { _, _ -> },
    onStop: (SshConfig, String) -> Unit = { _, _ -> },
    onRestart: (SshConfig, String) -> Unit = { _, _ -> },
    onStatus: (SshConfig, String) -> Unit = { _, _ -> },
    onLogs: (SshConfig, String) -> Unit = { _, _ -> },
    ...
    var serviceName by remember { mutableStateOf("voiceconfig-agent-node") }
        ...
        text = {
            Column(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(enabled = canConnect, onClick = { onList(currentConfig()) }) {
                        Text("列出服务")
                    }
                    TextButton(enabled = canConnect && serviceName.isNotBlank(), onClick = { onStart(currentConfig(), serviceName.trim()) }) {
                        Text("启动")
                    }
                    TextButton(enabled = canConnect && serviceName.isNotBlank(), onClick = { onStop(currentConfig(), serviceName.trim()) }) {
                        Text("停止")
                    }
                    TextButton(enabled = canConnect && serviceName.isNotBlank(), onClick = { onRestart(currentConfig(), serviceName.trim()) }) {
                        Text("重启")
                    }
                }
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    TextButton(enabled = canConnect && serviceName.isNotBlank(), onClick = { onStatus(currentConfig(), serviceName.trim()) }) {
                        Text("状态")
                    }
                    TextButton(enabled = canConnect && serviceName.isNotBlank(), onClick = { onLogs(currentConfig(), serviceName.trim()) }) {
                        Text("日志")
                    }
