...
> 状态：✅ 本轮已落地组件、功耗策略与本地 KWS 模型资产；仍待做 1 小时真机功耗验收
...
> 状态：✅ 核心链路已完成并在真机验证 trace；不可逆操作/二次确认实机演练待补
...
### 7.4 本轮 FlowScript / 降级平台化后的系统性判断

**终极目标未变，但实现路径更清晰了：**

> 不是“让 LLM 更聪明地操作手机”，而是“把可靠路径沉淀为可执行数据，
> 让 LLM 只做选择和异常处理，让安全门和验证层不依赖模型自觉”。

本轮已经完成：

- FlowScript 成为可导入/可审核/可版本化的确定性流程数据；
- `UiFlowExecutor` 统一执行，禁止动作由引擎强制；
- `run_flow_script` 让任意 APPROVED 脚本可通过 Agent 调用；
- 参数模板与 `InputText/Wait` 让脚本初步具备“可配置、可输入、可等待”；
- 强提醒与执行模式为无 Shizuku 熄屏提供了明确降级方向。

**当前主要矛盾：**

1. **平台有了，域只有瑞幸**。最大风险是“为了证明平台而继续加花活”，而不是先把第二个域真正跑通。
2. **确定性执行有了，但异常兜底没闭环**。FlowScript 只覆盖正常路径，遇到异常页面仍需 LLM 接管；必须定义“脚本失败 → 模型接管 → 人工/降级”的明确状态。
3. **熄屏提醒有了，续跑没有**。强提醒只能把用户叫醒，不能自动恢复原任务；需要 `WAITING_USER_WAKE` 状态。
4. **执行模式定义有了，调度没接入**。应让调度器主动选择 Assist/Ambient/Notify，而不是被动失败后提醒。
5. **安全与审核有了，但还缺少“证据链+SLA”**。每个 FlowScript 需要运行记录、成功率、禁止动作审计、真机 golden。

**下一阶段唯一主线：**

> 用 FlowScript 把两个真实域跑成“真机 golden 5/5 + 可观测 + 可安全停靠”，
> 然后从成功 trace 反向生成新脚本，形成“执行 → 验证 → 沉淀 → 审核 → 再执行”的闭环。
